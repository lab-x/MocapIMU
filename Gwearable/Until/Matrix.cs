    ...
    public class Matrix
    {
        ...
        public void MakeLU()                        // Function for LU decomposition
        {
            ...
            for (int k = 0; k < cols - 1; k++)
            {
                ...
                {
                    Console.WriteLine("The matrix is singular!"); //throw new MException("The matrix is singular!");
                    break;
                }
        ...
        public override string ToString()                           // Function returns matrix as a string
        {
            ...
            for (int i = 0; i < rows; i++)
            {
                for (int j = 0; j < cols; j++) s += String.Format("{0,5:0.0000000}", mat[i, j]) + " ";
                s += "\r\n";
            }
            return s;
        }
        ...
        public CQuaternion Matrix2Quat(Matrix RotMat)
        {
            CQuaternion q = new CQuaternion(1,0,0,0);
            double trace = (RotMat[0, 0] + RotMat[1, 1] + RotMat[2, 2]);
            if (trace > 0)
            {
                double s = 0.5 / System.Math.Sqrt(trace + 1.0f);
                q.m_q0 = 0.25 / s;
                q.m_q1 = (RotMat[2, 1] - RotMat[1, 2]) * s;
                q.m_q2 = (RotMat[0, 2] - RotMat[2, 0]) * s;
                q.m_q3 = (RotMat[1, 0] - RotMat[0, 1]) * s;
            }
            else
            {
                if (RotMat[0, 0] > RotMat[1,1] && RotMat[0,0] > RotMat[2, 2])
                {
                    double s = 2.0f * System.Math.Sqrt(1.0 + RotMat[0,0] - RotMat[1,1] - RotMat[2,2]);
                    q.m_q0 = (RotMat[2, 1] - RotMat[1, 2]) / s;
                    q.m_q1 = 0.25f * s;
                    q.m_q2 = (RotMat[0, 1] + RotMat[1,0]) / s;
                    q.m_q3 = (RotMat[0,2] + RotMat[2,0]) / s;
                }
                else if (RotMat[1,1] > RotMat[2,2])
                {
                    double s = 2.0 * System.Math.Sqrt(1.0 + RotMat[1,1] - RotMat[0,0] - RotMat[2,2]);
                    q.m_q0 = (RotMat[0, 2] - RotMat[2, 0]) / s;
                    q.m_q1 = (RotMat[0, 1] + RotMat[1, 0]) / s;
                    q.m_q2 = 0.25 * s;
                    q.m_q3 = (RotMat[1,2] + RotMat[2, 1]) / s;
                }
                else
                {
                    double s = 2.0 * System.Math.Sqrt(1.0 + RotMat[2,2] - RotMat[0,0] - RotMat[1,1]);
                    q.m_q0 = (RotMat[1,0] - RotMat[0,1]) / s;
                    q.m_q1 = (RotMat[0,2] + RotMat[2,0]) / s;
                    q.m_q2 = (RotMat[1,2] + RotMat[2,1]) / s;
                    q.m_q3 = 0.25 * s;
                }
            }
            return q;
