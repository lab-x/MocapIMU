...
namespace Gwearable
{
    ...
    {        
        public static string[] GetComName()
        {
            ...
            {
                if (s.Contains("G-Wearables Virtual COM Port") && !s.Contains("G-Wearables Virtual COM PortB"))
                {
                    int start = s.IndexOf("(");
                    int end = s.IndexOf(")");
                    string comName = Convert.ToString(s.Substring(start + 1, end - start - 1));
                    Portname[0] = comName;
                }

                if (s.Contains("G-Wearables Virtual COM PortB"))
                {
                    int start = s.IndexOf("(");
                    int end = s.IndexOf(")");
                    string comName = Convert.ToString(s.Substring(start + 1, end - start - 1));
